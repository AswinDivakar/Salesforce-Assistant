...
class AllDataBox extends React.PureComponent {
  ...
  loadSobjects() {
    ...
    function getEntityDefinitions(bucket) {
      console.log(bucket);
      let query = "select QualifiedApiName, Label, KeyPrefix, DetailUrl from EntityDefinition" + bucket;
...
class AllDataBoxSObject extends React.PureComponent {
  constructor(props) {
    ...
    this.fieldDataRender = this.fieldDataRender.bind(this);
  }
  ...
  getMatches(query) {
    ...
    console.log("DBG", contextRecordId);
  ...
  getFieldData(query) {
    ...
    let recordData = getRecordData(selectedValue);

    let res = Object.keys(recordData)
  ...
  getFieldSetupLink(sobjectName, field) {
    return "https://" + this.props.sfHost + "/lightning/setup/ObjectManager/" + sobjectName + "/FieldsAndRelationships/" + field + "/view";
  }

  fieldDataRender(matches, userQuery) {
    console.log("Data -> ", matches, userQuery);
    let {linkTarget} = this.props;
    let {selectedValue} = this.state;
    console.log("selectedValue", selectedValue);

    return matches.map(data => ({
      key: data.value + "#" + data.field,
      data,
      element: [
        h("div", {className: "autocomplete-item-main", key: "main"},
          h("a", {href: this.getFieldSetupLink(selectedValue.sobject.name, data.field), target: linkTarget}, data.field),
        ),
        h("div", {className: "autocomplete-item-sub", key: "sub"},
          data.value != null ? (typeof (data.value) == "object" ? "Composite Field" : data.value.toString()) : ""
        )
      ]
    }));
  }
  ...
  render() {
    ...
    return (
      h("div", {},
        h(AllDataSearch, {ref: "allDataSearch", onDataSelect: this.onDataSelect, sobjectsList, getMatches: this.getMatches, inputSearchDelay: 0, placeholderText: "Record Id or Object Name", resultRender: this.resultRender}),
        h("div", {className: "pt-2"},
          h(AllFieldsSearch, {ref: "allFieldsSearch", onFieldSelect: this.onFieldSelect, getFieldData: this.getFieldData, inputSearchDelay: 0, placeholderText: "Field Name", resultRender: this.fieldDataRender}),
        ),
        selectedValue
...
function getRecordData(selectedValue) {
  console.log(selectedValue);
  let data = sfConn.rest("/services/data/v" + apiVersion + "/" + "sobjects/" + selectedValue.sobject.name + "/" + selectedValue.recordId);
  data.then(res => {
    console.log(res);
    recordData = res;
  }).catch(err => {
        ...
        h("div", {className: "all-data-box-data"},
          h("table", {},
            h("tbody", {},
              ...
              // h("tr", {},
              //   h("input", {className: "col height-40 form-control", placeholder: "Filter Results", value: resultsFilter, onInput: () => { for (let field in recordData){ if (field.includes(resultsFilter)){ console.log(field, recordData[field]); } } }}),
              // ),
            ),
          ),
...
class AllFieldsSearch extends React.PureComponent {
  constructor(props) {
    super(props);
    this.state = {
      queryString: "",
  ...
  updateAllDataInput(value) {
    this.props.onFieldSelect(value);
    this.setState({queryString: ""});
    this.getMatchesDelayed("");
  }
  onAllDataArrowClick() {
  ...
  getMatchesDelayed(userQuery) {
    ...
    queryDelayTimer = setTimeout(async () => {
      let {getFieldData} = this.props;
      const matchingResults = await getFieldData(userQuery);
      await this.setState({matchingResults});
    }, inputSearchDelay);
