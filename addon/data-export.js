  ...
  doExport() {
    ...
    function batchHandler(batch) {
      return batch.catch(err => {
        ...
        vm.isWorking = false;
        vm.exportStatus = exportedData.records.length + (exportedData.records.length != exportedData.totalSize ? " of " + exportedData.totalSize : "") + " record(s).";
        vm.exportError = null;
        vm.exportedData = exportedData;
        vm.updatedExportedData();
        return null;
...
class App extends React.Component {
  ...
  render() {
    ...
    return h("div", {},
      ...
      h("div", {className: "area p-2 sticky-top"},
        h("h5", {}, "Export Query"),
        h("label", {},
        ...
        h("label", {},
          ...
          h("a", {href: "about:blank", onClick: this.onClearHistory, title: "Clear query history", className: "char-btn bad"}, "x")
        ),
        h("label", {},
          ...
          h("a", {href: "about:blank", onClick: this.onAddToHistory, title: "Add query to saved history", className: "char-btn good"}, "+"),
          h("a", {href: "about:blank", onClick: this.onRemoveFromHistory, title: "Remove Query From Saved", className: "char-btn bad"}, "x"),
          h("a", {href: "about:blank", onClick: this.onClearSavedHistory, title: "Clear All Saved Queries", className: "char-btn bad"}, "xx")
        ),
        h("a", {href: "about:blank", id: "export-help-btn", onClick: this.onToggleHelp}, "Help"),
        ...
        h("button", {className: "btn btn-primary action-button action-buton-center", disabled: model.isWorking, onClick: this.onExport, title: "Ctrl+Enter"}, "Export"),
      ),
      h("div", {className: "p-2", id: "result-area"},
        h("div", {className: "jumbotron-fluid area row p-2 result-bar"},
          h("div", {className: "col"},
            h("h5", {className: ""}, "Export Result"),
            h("span", {}, model.exportStatus),
          ),
          h("div", {className: "row col"},
            h("button", {className: "col btn btn-primary action-button", disabled: !model.canCopy(), onClick: this.onCopyAsExcel, title: "Copy exported data to clipboard for pasting into Excel or similar"}, "Copy Excel"),
            h("button", {className: "col btn btn-primary action-button", disabled: !model.canCopy(), onClick: this.onCopyAsCsv, title: "Copy exported data to clipboard for saving as a CSV file"}, "Copy CSV"),
            h("button", {className: "col btn btn-primary action-button", disabled: !model.canCopy(), onClick: this.onCopyAsJson, title: "Copy raw API output to clipboard"}, "Copy JSON"),
          ),
          h("input", {className: "col height-40 form-control", placeholder: "Filter Results", value: model.resultsFilter, onInput: this.onResultsFilterInput}),
          h("span", {className: "result-status"},
            h("button", {className: "btn btn-danger action-button ml-2", disabled: !model.isWorking, onClick: this.onStopExport}, "Stop")
          ),
        ),
        h("div", {className: "form-group shadow-textarea z-depth-1"},
          h("textarea", {className: "md-textarea col height-40 form-control", id: "result-text", readOnly: true, value: model.exportError || "", hidden: model.exportError == null}),
        ),
